class WelcomeController < ApplicationController
  ...
  def set_filter
    if params[:min_price].present? and params[:max_price].present? and params[:quantity].present? and params[:rating].present?
      ...
      @products = Product.where(admin_verified: true).where('end_at > ?', DateTime.now).order('id DESC').includes(:product_images)
      # @advertisements = Advertisement.where(admin_verified: true).order('id DESC')
      @services = Service.where(admin_verified: true).order('id DESC').includes(:service_images)
    end
  end
